...
class Timer:
    def __init__(self):
        self.t_0 = 0 # Initial value of the timer. It will be changed by the current geographic time the moment the timer is started
        self.started = False #If timer is started or not.
        
    def start_timer(self):
        self.t_0 = time.time() # Change the time value to the current geographic time to have a valid substraction when the 
        self.started = True
...
class Solenoid:
    def __init__(self, I, L, R):
        ...
        self.B= 0 # Initial value of B for B_z (Basically just a declaration)
    ...
    def _get_B_z(self, z, N):
        const = self.mu_0 * N * self.I/2
        fterm = ((self.L/2) - z) / (L*np.sqrt(self.R**2 + ((L/2) - z)**2))
        sterm = ((self.L/2) + z) / (L*np.sqrt(self.R**2 + ((L/2) + z)**2))
        self.B = const*(fterm + sterm)
        return self.B
    ...
    def _verif_if_within_tol(self, N):
        timeel = timer.get_timer()
        print(f'\rInitializing simulation... elapsed time: {timeel}s', end='\r')
